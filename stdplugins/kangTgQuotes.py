...
@borg.on(admin_cmd(pattern="kquote ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    me = borg.me
    userid = borg.uid
    packname = f"Telegram Quotes"
    packshortname = f"TgQuotes"  # format: Uni_Borg_userid
    ...
    if is_a_s:
        ...
        packname = f"Sid's animatedkangpack"
        packshortname = f"Sid_as_kangpack"  # format: Uni_Borg_userid
